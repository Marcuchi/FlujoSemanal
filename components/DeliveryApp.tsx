    ...
    <div className="h-full flex flex-col bg-slate-100 overflow-hidden print:overflow-visible print:h-auto print:bg-white">
      <style>{`
        @media print {
            ...
            /* Hide scrollbars and app wrappers */
            ::-webkit-scrollbar { display: none; }
            
            /* Global Table Styles for Print */
            table {
                width: 100%;
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar print:overflow-visible print:h-auto">
          ...
          <div className="max-w-7xl mx-auto p-4 md:p-8 print:p-0 print:max-w-none print:w-full">
            ...
            <div className="hidden print:block mb-2">
                 <div className="flex justify-between items-end border-b-2 border-black pb-2 mb-2">
                     <div>
                         ...
                         <div className="flex items-center gap-4 mt-1">
                             <div className="flex items-center gap-1">
                                 <span className="font-bold text-black text-[10px]">ZONA:</span>
                                 <span className="text-sm font-bold text-black uppercase leading-none">{zoneName}</span>
                             </div>
                             <div className="w-px h-3 bg-black"></div>
                             <div className="flex items-center gap-1">
                                 <span className="font-bold text-black text-[10px]">FECHA:</span>
                     ...
                     <div className="text-right">
                         <div className="text-[10px] font-bold text-black uppercase">Avícola Alpina</div>
                     </div>
                 </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200 print:bg-slate-100 print:border-black">
                                <th className="px-2 py-1 text-left text-sm print:text-[9px] print:py-0.5 font-bold text-slate-600 print:text-black uppercase tracking-wider border-r border-slate-200 print:border-slate-300">Cliente</th>
                                <th className="px-2 py-1 text-left text-sm print:text-[9px] print:py-0.5 font-bold text-slate-600 print:text-black uppercase tracking-wider w-36 print:w-24 border-r border-slate-200 print:border-slate-300">Articulo</th>
                                    ...
                                    <tr key={row.id} className={`hover:bg-slate-50 print:hover:bg-transparent group ${isAlternate ? 'print:bg-slate-50' : ''}`}>
                                        ...
                                        <td className="border-r border-slate-100 print:border-slate-300 h-8 print:h-[11px]">
                                             <NumericInput 
                                                ...
                                                className="text-slate-600 text-right font-mono text-sm font-medium print:text-[9px] print:leading-none"
                                                isCurrency
                                            />
                                        </td>
                                        <td className="border-r border-slate-100 print:border-slate-300 h-8 print:h-[11px] bg-emerald-50/30 print:bg-transparent">
                                             <NumericInput 
                                                ...
                                                className="text-emerald-700 print:text-black font-bold text-right font-mono text-sm print:text-[9px] print:leading-none"
                                                isCurrency
                                            />
                                        </td>
            ...
            {/* Expenses Section (Screen Only) */}
            <div className="mt-8 print:hidden flex flex-col md:flex-row gap-8 break-inside-avoid">
                {/* Expenses Table */}
            ...
            {/* PRINT ONLY FOOTER (Redesigned) */}
            <div className="hidden print:block mt-4 print-footer">
                 {/* Expenses Table (Excel Style) */}
                 <div>
                    <h4 className="font-bold text-xs uppercase mb-1 border-b border-black">Gastos</h4>
                    <table>
                        <thead>
                            <tr>
                                <th style={{textAlign: 'left'}}>Descripción</th>
                                <th style={{textAlign: 'right', width: '80px'}}>Monto</th>
                            </tr>
                        </thead>
                        <tbody>
                            {expenses.map(exp => (
                            ...
                            {expenses.length === 0 && <tr><td colSpan={2} style={{textAlign: 'center', fontStyle: 'italic'}}>Sin gastos</td></tr>}
                        </tbody>
                        <tfoot>
                            <tr>
                                <td style={{textAlign: 'right', fontWeight: 'bold'}}>TOTAL</td>
                                <td style={{textAlign: 'right', fontWeight: 'bold'}}>{formatCurrency(totalExpenses)}</td>
                            </tr>
                        </tfoot>
                    </table>
                 </div>
                 ...
                 <div>
                    <h4 className="font-bold text-xs uppercase mb-1 border-b border-black">Resumen</h4>
                    <table>
                        <tbody>
                            <tr>
                                <td style={{fontWeight: 'bold'}}>Vendido</td>
                                <td style={{textAlign: 'right'}}>{formatCurrency(totalSold)}</td>
                            </tr>
                            <tr>
                                <td style={{fontWeight: 'bold'}}>Gastos</td>
                                <td style={{textAlign: 'right'}}>{formatCurrency(totalExpenses)}</td>
                            </tr>
                            <tr>
                                <td style={{fontWeight: 'bold', fontSize: '12px'}}>EFECTIVO</td>
                                <td style={{textAlign: 'right', fontWeight: 'bold', fontSize: '12px'}}>{formatCurrency(cashBalance)}</td>
                            </tr>
                        </tbody>
                    </table>
                 </div>
