...
export const DailyReportModal: React.FC<DailyReportModalProps> = ({ isOpen, onClose, dayData }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-[90vw] max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 uppercase">Informe Diario: {dayData.name}</h2>
          </div>
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 custom-scrollbar bg-white dark:bg-slate-950">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
              ...
              <div className="flex flex-row justify-between items-center mb-6 border-b border-slate-200 dark:border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-amber-600 dark:text-amber-200 uppercase tracking-wider flex items-center gap-2">
                   <Users size={16} /> Detalle Adelantos
                </h3>
                <span className="text-lg font-mono font-bold text-amber-700 dark:text-amber-100 bg-amber-50 dark:bg-amber-900/30 px-3 py-1 rounded-lg border border-amber-200 dark:border-amber-900/50 shadow-sm">
                ...
                {salariesList.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {salariesList.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 hover:bg-slate-100 dark:hover:bg-slate-800/30 rounded transition-colors border-b border-slate-200 dark:border-slate-800/50 last:border-0">
                         <div className="flex items-center gap-3">
                           <span className="text-xs font-mono text-slate-500 w-4">{idx + 1}.</span>
                           <span className="text-slate-700 dark:text-slate-300 font-medium capitalize truncate">{item.name}</span>
                         </div>
                         <span className="text-amber-600 dark:text-amber-300 font-mono font-bold text-sm">
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end">
          <button 
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold rounded-lg transition-colors"
          >
            Cerrar
          </button>
        </div>
